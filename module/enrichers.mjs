...
export function createRollLabel(config) {
  ...
  if ( config.icon ) {
    switch ( config.type ) {
      case "check":
      case "skill":
        label = `<i class="dnd5e-icon" data-src="systems/las-torres-malditas/icons/svg/ability-score-improvement.svg"></i>${label}`;
        break;
      case "tool":
  ...
  // Roll request
    ...
    const chatData = {
      ...
      content: await foundry.applications.handlebars.renderTemplate(
        "systems/las-torres-malditas/templates/chat/roll-request-card.hbs", { buttons }
      ),
