...
export default class RollConfigurationDialog extends Dialog5e {
  ...
  static PARTS = {
    formulas: {
      template: "systems/las-torres-malditas/templates/dice/roll-formulas.hbs"
    },
    configuration: {
      template: "systems/las-torres-malditas/templates/dice/roll-configuration.hbs"
    },
    buttons: {
      template: "systems/las-torres-malditas/templates/dice/roll-buttons.hbs"
    }
  };
  ...
  _identifyDiceTerms() {
    ...
    const identifyTerm = term => {
      ...
      for ( let i = 0; i < term.number; i++ ) dice.push({
        icon: `systems/las-torres-malditas/icons/svg/dice/${term.denomination}.svg`,
