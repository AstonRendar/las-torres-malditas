...
export const getMigrationData = async function() {
  const data = {};
  try {
    const icons = await fetch("systems/las-torres-malditas/json/icon-migration.json");
    const spellIcons = await fetch("systems/las-torres-malditas/json/spell-icon-migration.json");
...
function _migrateTokenImage(actorData, updateData) {
  ...
  for ( const path of ["texture.src", "prototypeToken.texture.src"] ) {
    ...
    if ( oldSystemPNG.test(v) ) {
      ...
      updateData[path] = `systems/las-torres-malditas/tokens/${type}/${fileName}.webp`;
    }
  }
  return updateData;
}
